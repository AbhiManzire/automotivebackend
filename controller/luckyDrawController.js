...
const Coupon = require('../models/Coupon');
const LuckyDrawConfig = require('../models/LuckyDrawConfig');
const LuckyDrawCoupon = require('../models/LuckyDrawCoupon');
...
const getConfig = async (vehicleType, state) => {
  const config = await LuckyDrawConfig.findOne({ 
    vehicleType, 
    state, 
    status: 'active' 
  });
...
exports.manualGenerate = async (req, res) => {
  try {
    ...
    if (!["Cars", "Bikes"].includes(vehicleType)) {
      return res.status(400).json({ message: "Invalid vehicle type for lucky draw" });
    }
    ...
    const coupons = [];
    for (let i = 0; i < count; i++) {
      // Create LuckyDrawCoupon
      const ldCoupon = new LuckyDrawCoupon({
        couponCode: generateCouponCode(vehicleType, state),
        ...
        region,
        state,
        redeemAsDiscount: true,
        luckyDrawConfigId: config._id
      });
      await ldCoupon.save();

      // Create linked discount coupon in general coupon system
      const discountCoupon = new Coupon({
        title: { en: 'Lucky Draw Coupon' },
        couponCode: ldCoupon.couponCode,
        startTime: new Date(),
        endTime: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        discountType: "flat",
        discountValue: config.couponValue,
        minimumAmount: 0,
        productType: vehicleType,
        status: 'show',
      });
...
exports.autoGenerate = async (req, res) => {
  try {
    const config = await getConfig(req.body.vehicleType, req.body.state);
    if (config.mode !== "auto") {
      return res.status(400).json({ message: "Config is not in auto mode" });
    }
    return exports.manualGenerate(req, res);
  } catch (err) {
    res.status(400).json({ message: err.message });
...
exports.getUserCoupons = async (req, res) => {
  try {
    const coupons = await LuckyDrawCoupon.find({ userId: req.params.userId });
    res.status(200).json(coupons);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
exports.redeemCoupon = async (req, res) => {
  try {
    const coupon = await LuckyDrawCoupon.findOneAndUpdate(
      { _id: req.params.id, userId: req.user._id },
      { isRedeemed: true, redeemedAt: new Date() },
      { new: true }
    );
    if (!coupon) {
      return res.status(404).json({ message: "Coupon not found or not assigned to you" });
    }
